...
<div class="w-full flex items-center justify-center bg-auth flex-grow">
    <form class="rounded-xl flex flex-col p-16 bg-zinc-900 items-center gap-5 md:w-1/3 w-9/12 custom-shadow" action="{{route('user.create')}}" method="POST" enctype="multipart/form-data">
        @csrf
        <h4 class="text-3xl">Sign Up</h4>

        <div class="flex flex-col gap-3 items-start text-lg w-full justify-between">
            <label for="name">Name</label>
            <input type="text" name="name" id="name" class="border-2 border-sky-500 bg-transparent rounded-xl p-1 w-full">
        </div>
